...
// Универсальная функция для API запросов (исправленная)
async function apiRequest(endpoint, options = {}) {
    const apiKey = getApiKey();
    
    if (!apiKey) {
        throw new Error('Для получения доступа к API необходимо ...ции. Для этого нужно передать в запросе персональный API Key.');
    }

    // Формируем url с api_key в querystring
    const separator = endpoint.includes('?') ? '&' : '?';
    const url = `${API_BASE_URL}${endpoint}${separator}api_key=${encodeURIComponent(apiKey)}`;

    // Берём переданные опции, но явно подготовим headers
    const config = {
        ...options,
        headers: {
            ...(options.headers || {})
    ...
    // Если тело — объект (и не FormData), сериализуем в JSON и поставим правильный заголовок
    ...
    // показываем тело только в отладочных целях
    if (config.body) {
        console.log('Request body (truncated):', typeof config.body === 'string' ? config.body.slice(0, 500) : config.body);
    }
...
async function loadDishes() {
    try {
        ...
        console.log('Received dishes:', dishesFromApi); // Для отладки
        
        // Преобразуем категории из API в наши внутренние названия
        const categoryMap = {
            'main-course': 'main',
            'salad': 'starter', 
            'drink': 'drink',
